create
or replace function calculate_projected_data_kamada_v3(validation_id_params int) returns table (
  barangay text,
    voters bigint,
...
LEFT JOIN 
    ( SELECT brgy, COUNT(*) as voters_deceased
     ...
     ON e.brgy = vde.brgy      
GROUP BY 
    e.brgy, s.projected_winning_votes, s.max_teammembers_included_leader, t.team_count, organized_member,va.voters_ato,vd.voters_dili,vu.voters_undecided,vde.voters_deceased
ORDER BY 
    e.brgy ASC;
