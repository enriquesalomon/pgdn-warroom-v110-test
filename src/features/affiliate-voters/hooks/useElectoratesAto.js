...
import { getElectoratesGold } from "../../../services/apiElectorates";
...
export function useElectoratesAto(searchTerm) {
  ...
  const queryKey = ["electorates_gold", brgy, page, searchTerm];

  const { data, isPending: isLoading } = useQuery({
    queryKey,
    queryFn: async () => {
      const { data, count } = await getElectoratesGold({
        brgy,
        page,
        searchTerm,
      });
  ...
  useEffect(() => {
    ...
    if (page < pageCount) {
      queryClient.prefetchQuery({
        queryKey: ["electorates_gold", brgy, page - 1, searchTerm],
        queryFn: () =>
          getElectoratesGold({
            brgy,
            page: page - 1,
