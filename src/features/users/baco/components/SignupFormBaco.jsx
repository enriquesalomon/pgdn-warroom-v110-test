...
function SignupFormBaco({
  ...
  const leader_list = secondData
    ? secondData.map((baco) => ({
        value: baco.id,
        label: `${baco.firstname || ""} ${baco.middlename || ""} ${
          baco.lastname || ""
        } - ${baco.brgy || ""}`,
      }))
    : [];
  ...
  const onSubmit = (data) => {
    ...
    const action = !isEditSession
      ? "User created a new baco users account"
      : "User updated baco users account";
    const params = {
      page: "Users",
      action: action,
      parameters: data,
    ...
    data.baco_id = selectedLeader.value;
    if (isEditSession)
      editUser(
  ...
  return (
    ...
    >
      <div className="mb-12 flex text-m font-semibold px-16">
        {isEditSession ? "EDIT BACO USER" : "ADD NEW BACO USER"}
      </div>
