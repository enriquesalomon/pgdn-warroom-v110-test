...
import {
  // useElectorates,
  useSpecialElectorate,
} from "../hooks/useSpecialElectorate";
import SpecialElectorateRow from "./SpecialElectorateRow";
...
function SpecialElectorateTable() {
  const [exceldata, setExceldata] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [debouncedSearchTerm] = useDebounce(searchTerm, 500); // Debounce search term
  const { isPending, electorates, count } =
    useSpecialElectorate(debouncedSearchTerm);
  ...
  return (
    <Menus>
      ...
      <Table columns="0.5fr 1.8fr 1.8fr 1.8fr 1.8fr 1.8fr 1.8fr 1.8fr 1.8fr 1.8fr">
        <Table.Header>
          <div>#</div>
          <div>Electorate ID</div>
          <div>Precinct No.</div>
          <div>Lastname</div>
          <div>Firstname</div>
          <div>Middlename</div>
...
export default SpecialElectorateTable;
