...
export function useElectorate(searchTerm) {
  ...
  const queryKey = ["team_list_validation", brgy, page, searchTerm];
  ...
  useEffect(() => {
    ...
    if (page < pageCount) {
      queryClient.prefetchQuery({
        queryKey: ["team_list_validation", brgy, page - 1],
        queryFn: () =>
          getTeamList({
            brgy,
            page: page - 1,
...
export function useFetchAllData(shouldFetch) {
  ...
  const queryResult = useQuery({
    queryKey: ["team_list_validation_all", brgy],
...
const fetchSecondSelectData = async (precinctleader) => {
  let { data, error } = await supabase
    .from("electorates")
    .select(
      "id,precinctno,firstname,middlename,lastname,name_ext,purok,brgy,firstvalidation_tag,secondvalidation_tag,thirdvalidation_tag"
    )
