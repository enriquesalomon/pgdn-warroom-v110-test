...
function ElectoratesTable({ validation_name }) {
  ...
  console.log("mao ni val name", validation_name);
  ...
  const filteredData = electorates?.filter(
    (item) => item.islubas_type === "N/A"
  );

  console.log("non team electorates", JSON.stringify(electorates));
  ...
  return (
    <Menus>
      ...
      <div ref={componentRef}>
        ...
        >
          <h1>Non-team {validation_name}</h1>
          <p>Barangay: {brgy}</p>
          <p>Precinct No.: {precinct}</p>
        </div>

        {/* Wrap the component you want to print in the ref */}
        <Table columns="1fr 1fr 1.5fr 1.5fr 1.5fr 1fr 1.4fr 4.4fr">
          <Table.Header>
            {/* <div></div> */}
